...
app = Flask(__name__)
app.secret_key = 'medibase_secret_key_2025'  # Ganti dengan random string untuk production

# Konfigurasi database
app.config['MYSQL_HOST'] = 'localhost'
app.config['MYSQL_USER'] = 'root'
app.config['MYSQL_PASSWORD'] = ''
app.config['MYSQL_DB'] = 'medibase'
app.config['MYSQL_CURSORCLASS'] = 'DictCursor'

mysql = MySQL(app)

# -----------------------------------------------------------
# MIDDLEWARE - Check Login and Role
# -----------------------------------------------------------
# Catatan: login_required yang lama tidak lagi digunakan, tapi decorator baru sudah didefinisikan
# di atas. Kita akan menggunakan role_required untuk semua proteksi.

# Dekorator untuk memeriksa peran
def role_required(allowed_roles):
    def decorator(f):
        ...
        def decorated_function(*args, **kwargs):
            ...
            # Cek jika peran pengguna ada di daftar peran yang diizinkan
...
def login():
    if request.method == 'POST':
        ...
        if username == 'admin' and password == 'admin123':
            ...
            return redirect('/dashboard')
        elif username == 'petugas1' and password == 'petugas123':
            session['logged_in'] = True
            session['username'] = username
...
@app.route('/dashboard')
@role_required(['admin', 'petugas']) # Boleh diakses oleh Admin & Petugas
def dashboard():
    ...
    # Jadwal Dokter Hari Ini (contoh: semua dokter yang ada)
    # Anda bisa modifikasi query ini sesuai logika jadwal praktek
...
# -----------------------------------------------------------
# CRUD PASIEN (Akses: PETUGAS, READ: ADMIN)
# -----------------------------------------------------------

@app.route('/pasien')
@role_required(['admin', 'petugas']) # Boleh diakses oleh Admin (READ) & Petugas
def pasien():
    cur = mysql.connection.cursor()
...
# -----------------------------------------------------------
# CRUD OBAT (Akses: ADMIN, READ: PETUGAS)
# -----------------------------------------------------------
...
@role_required(['admin']) # Hanya Admin (CRUD)
def tambah_obat():
    if request.method == 'POST':
...
def hapus_obat(id):
    ...
    # HAPUS DULU DATA RESEP YANG PAKE OBAT INI
...
# -----------------------------------------------------------
# CRUD DOKTER (Akses: ADMIN, READ: PETUGAS)
# -----------------------------------------------------------
...
# -----------------------------------------------------------
# CRUD KUNJUNGAN (Akses: PETUGAS, READ: ADMIN)
# -----------------------------------------------------------
...
# -----------------------------------------------------------
# RIWAYAT PASIEN (Akses: ADMIN & PETUGAS)
# -----------------------------------------------------------
...
@role_required(['admin', 'petugas']) # <-- KOREKSI: Menggunakan role_required
def detail_riwayat(id_pasien):
    ...
    # 📌 2. Ambil riwayat kunjungan + obat (urut sesuai kunjungan)
    cur.execute("""
        SELECT 
            k.id_kunjungan,
            k.tanggal_kunjungan,
        ...
        LEFT JOIN obat o ON r.id_obat = o.id_obat
        WHERE k.id_pasien = %s
        ORDER BY k.id_kunjungan ASC      -- 🔥 urut sesuai data kunjungan
    """, (id_pasien,))
    riwayat = cur.fetchall()

    # 📌 3. Hitung total biaya per kunjungan (cursor BARU agar tidak error)
    total_biaya = {}
    sudah = set()
    ...
    for r in riwayat:
        ...
        if id_k not in sudah:
            cur2 = mysql.connection.cursor()     # ← pakai CURSOR BARU
            # Pastikan stored procedure HitungTotalBiaya tersedia di database Anda
            # Stored procedure ini harus menerima id_kunjungan dan mengembalikan Total_Biaya
            try:
                cur2.callproc("HitungTotalBiaya", (id_k,))
                hasil = cur2.fetchall()
                total_biaya[id_k] = hasil[0]["Total_Biaya"]
            except Exception as e:
                # Handle jika stored procedure tidak ada atau error
                print(f"Error calling HitungTotalBiaya for {id_k}: {e}")
                total_biaya[id_k] = 0 # Default ke 0 jika gagal
            finally:
                cur2.close()
...
# -----------------------------------------------------------
# REGISTRASI PASIEN (Akses: PETUGAS)
# -----------------------------------------------------------
